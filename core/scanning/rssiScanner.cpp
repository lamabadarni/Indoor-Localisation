...
void performRSSIScan() {
    ...
    for (int scan = 0; scan < RSSI_SCAN_BATCH_SIZE; ++scan) {
        ...
        LOG_INFO("RSSI", "Scan %d for label %s", scan + 1, labelToString(currentLabel));
        for (int i = 0; i < NUMBER_OF_ANCHORS; ++i) {
            LOG_INFO("RSSI", "RSSI[%d] = %d", i, scanData.RSSIs[i]);
        }
        Serial.println();
    }
}
...
int computeRSSIPredictionMatches() {
    ...
    for(int sampleToPredict = 0; sampleToPredict < NUM_OF_VALIDATION_SCANS; sampleToPredict++) {
        ...
        LOG_INFO("RSSI", "[VALIDATION] #%d: Predicted %s | Actual %s",
            sampleToPredict + 1, labelToString(predictedLabel), labelToString(currentLabel));
